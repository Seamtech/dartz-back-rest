...
export class AuthController {
  ...
  async login(
    ...
    const refreshToken = await this.tokenService.createTokensAndSetCookie(
      userProfile,
      response,
    );
  ...
  async editProfile(
    ...
    try {
      ...
      if (profileData.email || profileData.username) {
        ...
        await this.usersRepository.updateById(userId, userUpdates, {
          transaction,
        });
      }

      // Update player profile
      await this.playerProfileRepository.updateById(
        existingProfile.id,
        profileData,
        {transaction},
      );
